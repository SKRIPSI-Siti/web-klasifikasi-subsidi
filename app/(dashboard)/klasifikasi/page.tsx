  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold">Riwayat Klasifikasi</h2>
          <p className="text-sm text-muted-foreground">
            Semua klasifikasi yang pernah dijalankan.
          </p>
        </div>
      ...
      {state.predictions.length === 0 ? (
        <EmptyState
          ...
          description="Jalankan klasifikasi pertama menggunakan model aktif."
          action={
            <Button render={<Link href="/klasifikasi/baru" />}>
              <IconPlus />
              Klasifikasi Baru
              ...
              {state.predictions.map((p) => {
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant={p.jenis === "batch" ? "secondary" : "outline"}
                      >
                    ...
                    <TableCell>
                      <span className="text-success">
                        {p.jumlah_layak} Layak
                      </span>
                      {" / "}
                      <span className="text-destructive">
                        {p.jumlah_tidak} Tidak Layak
                      </span>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button
                        variant="outline"
                        size="sm"
                        render={<Link href={`/klasifikasi/hasil/${p.id}`} />}
                      >
                        Lihat
                      </Button>
                    </TableCell>
