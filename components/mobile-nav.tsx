...
const items = [
  ...
  // Slot tengah: aksi utama (Prediksi Baru) — dirender terpisah di bawah
  { title: "Riwayat", url: "/prediksi", icon: IconHistory, exclude: ["/prediksi/baru"] },
  { title: "Laporan", url: "/laporan", icon: IconReport },
]

/** Floating bottom bar untuk mobile (<768px) — halaman inti + CTA Prediksi Baru di tengah. */
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-md items-center gap-1 rounded-2xl border bg-popover/90 px-2 py-1 shadow-lg backdrop-blur supports-backdrop-filter:bg-popover/75">
        ...
        {/* CTA utama: Prediksi Baru */}
        <Link
          href="/prediksi/baru"
          aria-label="Prediksi Baru"
          className={cn(
            ...
            pathname.startsWith("/prediksi/baru") && "ring-3 ring-primary/30"
          )}
        >
